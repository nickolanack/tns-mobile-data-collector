			...
			var addVideo = function() {
				...
				videorecorder.record(options)
					.then((data) => {
						...
						me._renderer.renderButtonset(mediaSelection, {
							"buttons": [{
								"icon": "video-icon",
					...
					"fields": [{
						...
						// },
						{
							...
							"action": function(data) {
								...
								console.log("Main data: " + JSON.stringify(me._renderer.getCurrentFormData(), null, "   "));
	...
	return me._renderer.renderButtonset(container, {
		...
		"buttons": [{
			...
			"action": function(d, el) {
					...
					}).catch(function(e){ console.log ('player error '+e);});
...
MediaViewRenderer.prototype._createInlineVideo = function(field) {
	...
	if (src[0] !== "~") {
		src = (src.indexOf("https")!==0?'https://' + global.client.getUrl() + "/":"") + src;
